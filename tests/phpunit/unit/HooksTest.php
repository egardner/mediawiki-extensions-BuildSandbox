...
namespace MediaWiki\Extension\BuildSandbox\Tests;

use MediaWiki\Extension\BuildSandbox\Hooks;

/**
 * @coversDefaultClass \MediaWiki\Extension\BuildSandbox\Hooks
 */
class HooksTest extends \MediaWikiUnitTestCase {
	...
	public function testOnBeforePageDisplayVandalizeIsTrue() {
		$config = new \HashConfig( [
			'BuildSandboxVandalizeEachPage' => true
		] );
		...
		$outputPageMock->expects( $this->once() )
			->method( 'addHTML' )
			->with( '<p>BuildSandbox was here</p>' );
		$outputPageMock->expects( $this->once() )
			->method( 'addModules' )
			->with( 'oojs-ui-core' );
	...
	public function testOnBeforePageDisplayVandalizeFalse() {
		$config = new \HashConfig( [
			'BuildSandboxVandalizeEachPage' => false
		] );
		$outputPageMock = $this->getMockBuilder( \OutputPage::class )
			->disableOriginalConstructor()
			->getMock();
